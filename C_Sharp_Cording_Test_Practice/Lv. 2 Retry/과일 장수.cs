...
namespace C_Sharp_Cording_Test_Practice
{
    ...
    public class Solution
    {
        public int solution(int k, int m, int[] score)
        {
            ...
            {
                // 현재 인덱스가 0이 아니고, 한 상자에 담길 사과 개수인 m의 배수인 경우
                if (i != 0 && (i + 1) % m == 0)
                {
                    // 최저 점수 * 한 상자에 들어가는 사과 개수를 이익에 더함
                    answer += m * score[i];
                }
            }
            return answer;
    ...
    class 과일_장수
    {
        static void Main(string[] args)
        {
            ...
            int[] _score01 = { 1, 2, 3, 1, 2, 3, 1 };
            int[] _score02 = { 4, 1, 2, 2, 4, 4, 4, 4, 1, 2, 4, 2 };

            Console.WriteLine(_solution.solution(3, 4, _score01));
