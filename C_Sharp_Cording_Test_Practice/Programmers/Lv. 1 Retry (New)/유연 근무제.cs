 ...
 * 직원들이 일주일 동안 출근한 시각을 담은 2차원 정수 배열 timelogs, 이벤트를 시작한 요일을 의미하는 정수 startday가 매개변수로 주어집니다. 
...
namespace C_Sharp_Cording_Test_Practice.Programmers
{
    ...
    {
        //  scheduls : 직원 n명이 설정한 출근 희망 시각을 담은 1차원 정수 배열
        //  timeglos : 직원들이 일주일 동안 출근한 시각을 담은 2차원 정수 배열
        //  startday : 이벤트를 시작한 요일을 위미하는 정수
        public int solution(int[] schedules, int[,] timelogs, int startday)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int d = 0; d < 7; d++)
                {
                    int _dayOfWeek = (startday - 1 + d) % 7 + 1; // 1~7 (월~일)
                    ...
                    // 시간 형식 변환
                    TimeSpan _scheduleTime = IntToTimeSpan(schedules[i]);
                    TimeSpan _logTime = IntToTimeSpan(timelogs[i, d]);

                    // 출근 기준 + 10분 이내인지
                    if (_logTime <= _scheduleTime.Add(TimeSpan.FromMinutes(10)))
                        _validDays++;
                }
        ...
        // helper 함수: int → TimeSpan (예: 930 → 09:30)
        // TimeSpan : 시간 간격을 나타내는 구조체 (C# 지원 라이브러리)
        private TimeSpan IntToTimeSpan(int time)
        {
            int _hour = time / 100;
            int _minute = time % 100;
            TimeSpan _ts = TimeSpan.FromHours(_hour) + TimeSpan.FromMinutes(_minute);
            return _ts;
        }
    }
    internal class 유연_근무제
    {
        static void Main(string[] args)
        {
            Solution _solution = new Solution();

            int[] _schedules01 = { 700, 800, 1100 };
            int[] _schedules02 = { 730, 855, 700, 720 };

            int[,] _timelogs01 = new int[,]
        {
            ...
            Console.WriteLine(_solution.solution(_schedules01, _timelogs01, 5));     //  3
            Console.WriteLine(_solution.solution(_schedules02, _timelogs02, 1));     //  2
        }
    }
    */
}
